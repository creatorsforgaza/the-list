...
final_table <-  x %>% 
  ...
  dplyr::select(uuid, link, link_text,
                total_num_donations,currency_abv, amount_raised_num, 
                percent_of_goal, 
                total_num_donations_24_hours:money_raised_24_hours)

date <- format(lubridate::floor_date(Sys.Date(), unit = 'day'), format = '%Y-%m-%d')
readr::write_csv(final_table , glue::glue('data/final-table-{date}.csv'))
...
vbs <- list(
  ...
  value_box(
    title = paste0("Amount raised in Euros on ", format(lubridate::now("UTC") - hours(7), "%B %d, %Y"),
  ...
  value_box(
    title = paste0("Amount raised in US Dollars on ", 
                   format(lubridate::now("UTC") - hours(7), "%B %d, %Y"),
  ...
  value_box(
    title = paste0("Total amount raised in Pounds from ", num_funds_gbp, " funds"),
    # theme = "primary",
    value = paste0("£", sum_all_donations_in_gbp),
    icon = "cash"
  ),
  value_box(
    title = paste0("Amount raised in Pounds on ", 
                   format(lubridate::now("UTC") - hours(7), "%B %d, %Y"),
  ...
  value_box(
    ...
    # theme = "orange", # doesnt work must change this in scss file
    value = paste0("k", sum_all_donations_in_sek_24),
    icon = bs_icon("cash")
  )  
)
...
final_table %>% 
  ...
  gt::cols_label(
    ...
    total_num_donations_24_hours = paste0("Number of donations on ",format(lubridate::now("UTC") - hours(7), "%B %d")),
    money_raised_24_hours = paste0("Amount raised on ",format(lubridate::now("UTC") - hours(7), "%B %d")),
    # uuid = "QR Code",
